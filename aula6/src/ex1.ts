...
class Pessoa {
    constructor(private _nome: string, private _idade: number) { //diz diretamente quem são os atributos no construtor
        if (this._nome.length == 0) throw new Error('Nome invalido'); //exceção
        if (this._idade < 0) throw new Error('Idade invalido'); //exceção
    }
