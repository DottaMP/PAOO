class GSet<A> {
    ...
    constructor(private equals: (a: A, b: A) => boolean) {// cria a função equals e ensina como irá se comportar a igualdade
...
function test(){
    ...
    const set = new GSet<Box>((a, b) => Math.abs(a.value) == Math.abs(b.value));
    set.add(new Box(10));
    set.add(new Box(20));
...
function cnt<A>(xs: A[], eq: (a: A, b: A) => boolean, k: A): number{
    let c = 0;
    for(const x of xs){
        if(eq(k, x)) {
            ++c;
